    ...
    pipeline {
        ...
        stages{
            ...
            stage('打包前端NicelooSsrUI'){
                ...
                steps {
                   sh "${map.NPM_PACKAGE_COMMAND}"
                }
            }

            stage('部署前端NicelooSsr节点'){
                when { 
                    anyOf { 
                            environment name: 'DEPLOY_TYPE', value: 'test' ; 
                    } 
                ...
                steps {
                    script {
                        if (params.DEPLOY_TYPE == "test") {
                            ...
                            sh "salt ${map.TEST_IP1} cmd.run '${map.PM2_DEPLOY_COMMAND}'"
                        }
