    ...
    pipeline {
        ...
        stages{
            ...
            stage('清理前端DevOpsUI构建'){
                ...
                steps {
                    sh "echo ${map.CLEAN_COMMAND}"
                    sh "${map.CLEAN_COMMAND}"
                }
            }
            
            stage('构建前端DevOpsUI代码'){
                when { 
                    anyOf { 
                        environment name: 'DEPLOY_TYPE', value: 'test' 
                    }
            ...
            stage('部署前端DevOpsUI节点'){
                ...
                steps {
                    script {
                       ...
                       ZIPNAME = sh(script: 'ls build/ |grep zip', returnStdout: true).trim()
                       sh "salt ${map.TEST_IP1} cp.get_file salt://${JOB_NAME}/build/${ZIPNAME} ${map.DEPLOY_DIR}/${map.ZIP_NAME}"
